...
export type RunInspectorResult = Omit<AuditRunInspectResult, "decisions" | "decisionDisplays"> & {
  decisionDisplays: DecisionReceiptDisplayV1[];
};

export function toRunInspectorResult(result: AuditRunInspectResult): RunInspectorResult {
  const { decisions: _rawDecisions, decisionDisplays = [], ...safeResult } = result;
  return { ...safeResult, decisionDisplays };
}
