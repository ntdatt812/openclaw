...
describe("receipt paging model", () => {
  ...
  it("drops raw receipts at the Activity RPC boundary", () => {
    const result = unavailable("unknown", "run_not_found");
    const projected = toRunInspectorResult({
      ...result,
      decisions: [],
      decisionDisplays: result.decisionDisplays,
    } satisfies AuditRunInspectResult);
    expect(projected).not.toHaveProperty("decisions");
    expect(projected.decisionDisplays).toEqual([]);
  });
});
